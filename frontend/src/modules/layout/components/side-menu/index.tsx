              ...
              >
                <PopoverPanel className="flex flex-col absolute w-full pr-4 sm:pr-0 sm:w-1/3 2xl:w-1/4 sm:min-w-min h-[calc(100vh-1rem)] z-30 inset-x-0 text-sm text-white m-2 backdrop-blur-2xl">
                  <div
                    ...
                    className="flex flex-col h-full bg-black rounded-lg justify-between p-6"
                  >
                      ...
                      {Object.entries(SideMenuItems).map(([name, href]) => {
                        return (
                          <li key={name}>
                            <LocalizedClientLink
                              href={href}
                              className="text-3xl leading-10 hover:text-gray-400 text-white"
                              onClick={close}
                    ...
                    <div className="flex flex-col gap-y-6">
                      ...
                      <Text className="flex justify-between txt-compact-small text-gray-400">
                        © {new Date().getFullYear()} Novastore. All rights
                        reserved.
                      </Text>
                    </div>
                  </div>
